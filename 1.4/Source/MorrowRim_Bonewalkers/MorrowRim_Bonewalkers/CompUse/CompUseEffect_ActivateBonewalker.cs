...
{
    public class CompUseEffect_ActivateBonewalker : CompUseEffect
    {
		...
		public void SetQuality(Pawn servant, Pawn usedBy)
		{
			...
			q /= 10f;
			//Check optionals
			if (!Props.qualityHediffDefs.NullOrEmpty())
            {
				foreach(HediffDef def in Props.qualityHediffDefs)
                {
                    if (usedBy.health.hediffSet.HasHediff(def))
                    {
						q += 0.1f;
					}
                }
